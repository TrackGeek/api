...
export class JikanService {
  ...
  async getAnimeById(id: number): Promise<any> {
    try {
      ...
      const relations = animeFullData.relations ? animeFullData.relations.map((relation) => ({
      ...
      const characters = charactersData.map((char) => ({
        ...
        role: char.role,
        voiceActors: char.voice_actors ? char.voice_actors.map((va) => ({
      ...
      const videos = {
        promo: videosData.promo ? videosData.promo.map((promo) => ({
          title: promo.title,
          video: {
        ...
        episodes: videosData.episodes ? videosData.episodes.map((video) => ({
          malId: video.mal_id,
          title: video.title,
          episode: video.episode,
          imageUrl: video.images?.jpg?.image_url ?? null,
        })) : [],
        musicVideos: videosData.music_videos ? videosData.music_videos.map((musicVideo) => ({
      ...
      const anime = {
        ...
        producers: animeFullData.producers ? animeFullData.producers.map((producer) => ({
          malId: producer.mal_id,
          type: producer.type,
          name: producer.name,
        })) : [],
        licensors: animeFullData.licensors ? animeFullData.licensors.map((licensor) => ({
          malId: licensor.mal_id,
          type: licensor.type,
          name: licensor.name,
        })) : [],
        studios: animeFullData.studios ? animeFullData.studios.map((studio) => ({
          malId: studio.mal_id,
          type: studio.type,
          name: studio.name,
        })) : [],
  ...
  async getMangaById(id: number): Promise<any> {
    try {
      ...
      const relations = mangaFullData.relations ? mangaFullData.relations.map((relation) => ({
        relationType: relation.relation,
        entry: relation.entry.map((entry) => ({
          malId: entry.mal_id,
          title: entry.title,
      ...
      const manga = {
        ...
        authors: mangaFullData.authors ? mangaFullData.authors.map((author) => ({
          malId: author.mal_id,
          type: author.type,
          name: author.name,
        })) : [],
        serializations: mangaFullData.serializations ? mangaFullData.serializations.map((serialization) => ({
