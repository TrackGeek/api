...
export class JikanService {
	...
	async searchAnimes(query: string) {
		try {
			const cachedAnimes = await this.cacheService.get(
	...
	async searchMangas(query: string) {
		try {
			const cachedMangas = await this.cacheService.get(
	...
	async getAnimeById(id: number): Promise<any> {
		try {
			const cachedAnime = await this.cacheService.get(
			...
			const anime = {
				...
				studios: animeFullData.studios
					...
					: [],
				genres: animeFullData.genres
					? animeFullData.genres.map((genre) => genre.name)
					: [],
				explicitGenres: animeFullData.explicit_genres
					? animeFullData.explicit_genres.map((genre) => genre.name)
					: [],
				themes: animeFullData.themes
					? animeFullData.themes.map((theme) => theme.name)
					: [],
				demographics: animeFullData.demographics
					? animeFullData.demographics.map((demo) => demo.name)
					: [],
				external: animeFullData.external
					? animeFullData.external.map((ext) => ext.name)
					: [],
				characters,
				cast,
				videos,
	...
	async getMangaById(id: number): Promise<any> {
		try {
			const cachedManga = await this.cacheService.get(
			...
			const [mangaFullResponse, charactersResponse] =
				await manyRequestWithDelay({
					httpService: this.httpService,
					urls: [
						`${this.JIKAN_API_URL}/manga/${id}/full`,
						`${this.JIKAN_API_URL}/manga/${id}/characters`,
					],
				});
			...
			const manga = {
				...
				serializations: mangaFullData.serializations
					...
					: [],
				genres: mangaFullData.genres
					? mangaFullData.genres.map((genre) => genre.name)
					: [],
				explicitGenres: mangaFullData.explicit_genres
					? mangaFullData.explicit_genres.map((genre) => genre.name)
					: [],
				themes: mangaFullData.themes
					? mangaFullData.themes.map((theme) => theme.name)
					: [],
				demographics: mangaFullData.demographics
					? mangaFullData.demographics.map((demo) => demo.name)
					: [],
				external: mangaFullData.external
					? mangaFullData.external.map((ext) => ext.name)
					: [],
				characters,
				relations,
			};
