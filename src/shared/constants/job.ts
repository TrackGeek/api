export const ACTIVITY_JOB = "activity-job";

export const WATCHED_ACTIVITY_JOB = "watched-activity-job";

export const ACTIVITY_CLEANUP_JOB = "activity-cleanup-job";

export const ACTIVITY_CLEANUP_SCHEDULER_ID = "activity-cleanup-scheduler";

export const NOTIFICATION_SYSTEM_JOB = "notification-system-job";

export const NOTIFICATION_COMMENT_JOB = "notification-comment-job";

export const NOTIFICATION_REACTION_JOB = "notification-reaction-job";

export const MAGIC_LINK_JOB = "magic-link-job";

export const RESET_PASSWORD_JOB = "reset-password-job";

export const DELETE_ACCOUNT_JOB = "delete-account-job";

export const PAYMENT_SUCCESS_JOB = "payment-success-job";

export const SUBSCRIPTION_CANCELLED_JOB = "subscription-cancelled-job";

export const PAYMENT_FAILED_JOB = "payment-failed-job";

export const CATCHUP_DAILY_JOB = "catchup-daily-job";

export const CATCHUP_DAILY_SCHEDULER_ID = "catchup-daily-scheduler";

export const XP_JOB = "xp-job";
