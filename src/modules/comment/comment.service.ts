...
export class CommentService {
	...
	async getCommentsByBookId(bookId: string) {
		...
		const pagination =
			await this.databaseService.cursorPagination<BookCommentFindManyArgs>({
				model: "bookComment",
				where: { bookId },
				include: {
					comment: {
	...
	async getCommentsByAnimeId(animeId: string) {
		...
		const pagination =
			await this.databaseService.cursorPagination<AnimeCommentFindManyArgs>({
				model: "animeComment",
				where: { animeId },
				include: {
					comment: {
	...
	async getCommentsByMangaId(mangaId: string) {
		...
		const pagination =
			await this.databaseService.cursorPagination<MangaCommentFindManyArgs>({
				model: "mangaComment",
				where: { mangaId },
				include: {
					comment: {
	...
	async getCommentsByMovieId(movieId: string) {
		...
		const pagination =
			await this.databaseService.cursorPagination<MovieCommentFindManyArgs>({
				model: "movieComment",
				where: { movieId },
				include: {
					comment: {
