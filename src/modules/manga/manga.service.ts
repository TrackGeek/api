...
export class MangaService {
	...
	async getMangaById(id: number) {
		const cachedManga = await this.cacheService.get<Manga>(
			this.cacheKeys.mangaById.prefix(id),
		);
	...
	async refreshManga(refreshMangaDto: RefreshMangaDto) {
		...
		const jikanManga = await this.integrationsService.jikan.getMangaById(
			manga.malId,
		);
