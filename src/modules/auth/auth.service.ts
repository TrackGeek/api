...
export class AuthService {
  ...
  async generateEmailLoginUrl(emailLoginDto: RequestEmailLoginDto) {
  ...
  async loginWithEmail(loginDto: LoginWithEmailDto) {
    ...
    try {
      ...
      const user = await this.createOrGetUser(email, this.extractNameFromEmail(email));
  ...
  async generateGoogleLoginUrl() {
    ...
    return decodeURI(`https://accounts.google.com/o/oauth2/v2/auth?${query.toString()}`);
  }
  ...
  async loginWithGoogle(loginDto: LoginWithGoogleDto) {
    ...
    const googleTokenResponse = await this.httpService.axiosRef
      ...
      .catch((err) => {
        this.logger.error('Error fetching Google tokens', err.response?.data ?? err.message);
    ...
    const googleUserResponse = await this.httpService.axiosRef
      ...
      .catch((err) => {
        this.logger.error('Error fetching Google user info', err.response?.data ?? err.message);
    ...
    const user = await this.createOrGetUser(googleUserResponse.email, googleUserResponse.name);
  ...
  async generateDiscordLoginUrl() {
    ...
    return decodeURI(`https://discord.com/oauth2/authorize?${query.toString()}`);
  }
  ...
  async loginWithDiscord(loginDto: LoginWithDiscordDto) {
    ...
    const discordTokenResponse = await this.httpService.axiosRef
      .post("https://discord.com/api/v10/oauth2/token", new URLSearchParams({
        client_id: process.env.DISCORD_CLIENT_ID!,
      ...
      .catch((err) => {
        this.logger.error('Error fetching Discord tokens', err.response?.data ?? err.message);
    ...
    const discordUserResponse = await this.httpService.axiosRef
      ...
      .catch((err) => {
        this.logger.error('Error fetching Discord user info', err.response?.data ?? err.message);
    ...
    const user = await this.createOrGetUser(discordUserResponse.email, discordUserResponse.name);
  ...
  async generateGithubLoginUrl() {
    ...
    return decodeURI(`https://github.com/login/oauth/authorize?${query.toString()}`);
  }
  ...
  async loginWithGithub(loginDto: LoginWithGithubDto) {
    ...
    const githubTokenResponse = await this.httpService.axiosRef
      ...
      .catch((err) => {
        this.logger.error('Error fetching GitHub tokens', err.response?.data ?? err.message);
    ...
    const githubUserResponse = await this.httpService.axiosRef
      ...
      .catch((err) => {
        this.logger.error('Error fetching GitHub user info', err.response?.data ?? err.message);
    ...
    const githubEmailResponse = await this.httpService.axiosRef
      ...
      .then((response) => {
        const primaryEmail = response.data.find((e: any) => e.primary)?.email ?? response.data[0]?.email;
        
        return primaryEmail;
      })
      .catch((err) => {
        this.logger.error('Error fetching GitHub emails', err.response?.data ?? err.message);
    ...
    const user = await this.createOrGetUser(githubEmailResponse, githubUserResponse.name);
